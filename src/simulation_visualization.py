"""
Visualisations individuelles — Méthodologie Cadoux (2004)
=========================================================
Génère chaque graphique séparément dans le même dossier.
...
# ════════════════════════════════════════════════════════════
# PRÉPARATION DES DONNÉES (commune à toutes les figures)
# ════════════════════════════════════════════════════════════
...
for (i,j), plabel in zip(pairs, pair_labels):
    ...
    dep_results.append({'Paire': plabel, 'Kendall τ': round(tau,4),
                        'p-val': round(p_tau,4), 'Spearman ρ': round(rho,4),
...
# ════════════════════════════════════════════════════════════
# FIGURE 1 — Séries temporelles mensuelles
# ════════════════════════════════════════════════════════════
def fig1_series_temporelles():
    ...
    plt.close()
    print(f"  ✓ fig1_series_temporelles.png")
...
def fig2_marginales():
    ...
    for k, (c, label, color) in enumerate(zip(cols_adj, labels, ACC8)):
        ...
        ax.set_xlabel('M€ / mois', color=TEXT, fontsize=9)
        ax.set_ylabel('Densité', color=TEXT, fontsize=9)
    ...
    plt.savefig(path, dpi=150, bbox_inches='tight', facecolor=DARK)
    plt.close()
    print(f"  ✓ fig2_marginales.png")

# ════════════════════════════════════════════════════════════
# FIGURE 3 — Matrice Kendall τ (heatmap 8×8)
# ════════════════════════════════════════════════════════════
def fig3_heatmap_kendall():
    ...
    ax.set_title('Matrice Kendall τ — 8 garanties (28 paires)\n'
                 'Valeurs en gras = dépendance significative (|τ| > 0.10)',
    ...
    plt.savefig(path, dpi=150, bbox_inches='tight', facecolor=DARK)
    plt.close()
    print(f"  ✓ fig3_heatmap_kendall.png")
...
def fig4_scatter_copules():
    ...
    for ax, plabel, color in zip(axes, top3, pair_colors):
        ...
        tau_v = dep_df[dep_df['Paire']==plabel]['Kendall τ'].values[0]
        ...
        ax.set_title(f'{plabel}\nτ = {tau_v:.3f}  |  Copule : {cop_v.capitalize()}',
    ...
    fig.suptitle('Espace des copules — 3 paires avec la dépendance la plus forte',
                 color=TEXT, fontsize=13, fontweight='bold', y=1.02)
    path = os.path.join(SCRIPT_DIR, 'fig4_scatter_copules.png')
    plt.tight_layout()
    plt.savefig(path, dpi=150, bbox_inches='tight', facecolor=DARK)
    plt.close()
    print(f"  ✓ fig4_scatter_copules.png")

# ════════════════════════════════════════════════════════════
# FIGURE 5 — Copule sélectionnée par paire (barres horizontales)
# ════════════════════════════════════════════════════════════
def fig5_copules_bar():
    ...
    ax.set_title('Copule sélectionnée par paire significative\n(critère AIC, méthode CML)',
                 color=TEXT, fontsize=12, fontweight='bold', pad=12)
    ax.tick_params(colors=TEXT)
    ...
    plt.savefig(path, dpi=150, bbox_inches='tight', facecolor=DARK)
    plt.close()
    print(f"  ✓ fig5_copules_bar.png")

# ════════════════════════════════════════════════════════════
# FIGURE 6 — Distribution perte agrégée + seuils VaR
# ════════════════════════════════════════════════════════════
def fig6_distribution_agregee():
    ...
    ax.hist(total[total<=pct]/1e6, bins=150, density=True,
            ...
            label='Scénarios simulés (100 000)')
    ...
    for cv, res in zip(cmap_v, cap_results):
        ...
        if v/1e6 <= pct/1e6:
            ax.axvline(v/1e6, color=cv, lw=2, linestyle='--',
                       label=f"VaR {res['Seuil']} = {v/1e6:.1f} M€")
    ...
    ax.set_title('Distribution de la charge agrégée mensuelle (8 garanties)\n'
                 'Simulation Monte Carlo — Vine Copule',
                 color=TEXT, fontsize=12, fontweight='bold', pad=12)
    ax.set_xlabel('Charge totale mensuelle (M€)', color=TEXT, fontsize=11)
    ax.set_ylabel('Densité', color=TEXT, fontsize=11)
    ax.tick_params(colors=TEXT)
    ...
    plt.savefig(path, dpi=150, bbox_inches='tight', facecolor=DARK)
    plt.close()
    print(f"  ✓ fig6_distribution_agregee.png")

# ════════════════════════════════════════════════════════════
# FIGURE 7 — VaR vs TVaR vs Indépendance (barres groupées)
# ════════════════════════════════════════════════════════════
def fig7_capital_var_tvar():
    ...
    ax.bar(xp+w, [r['VaR_indep']/1e6 for r in cap_results],
           w, color='#95a5a6', label='VaR (indépendance)', alpha=0.9)
    ...
    ax.set_ylabel('M€ / mois', color=TEXT, fontsize=11)
    ax.set_title('Besoins en fonds propres — VaR vs TVaR vs Hypothèse d\'indépendance\n'
                 'Les % indiquent le gain de diversification de la copule vs indépendance',
                 color=TEXT, fontsize=12, fontweight='bold', pad=12)
    ax.tick_params(colors=TEXT)
    ...
    plt.savefig(path, dpi=150, bbox_inches='tight', facecolor=DARK)
    plt.close()
    print(f"  ✓ fig7_capital_var_tvar.png")

# ════════════════════════════════════════════════════════════
# FIGURE 8 — Tableau récapitulatif des résultats
# ════════════════════════════════════════════════════════════
def fig8_tableau_recap():
    ...
    # Tableau 2 — Top dépendances
    top8 = sig_df.head(8)
    data2 = [[f"{row['Kendall τ']:.3f}", f"{row['p-val']:.4f}",
              f"{row['Spearman ρ']:.3f}"]
             for _, row in top8.iterrows()]
    make_table(axes[1], data2,
               ['Kendall τ','p-val','Spearman ρ'],
               [p[:22] for p in top8['Paire']],
               'Top 8 dépendances', None)
    ...
    make_table(axes[2], data3,
               ['VaR','TVaR','VaR indép.','Gain %'],
               [r['Seuil'] for r in cap_results],
               'Capital (M€/mois)', None)

    fig.suptitle('Résumé des résultats — Méthodologie Cadoux (2004)',
    ...
    plt.savefig(path, dpi=150, bbox_inches='tight', facecolor=DARK)
    plt.close()
    print(f"  ✓ fig8_tableau_recap.png")

# ════════════════════════════════════════════════════════════
# LANCEMENT DE TOUTES LES FIGURES
# ════════════════════════════════════════════════════════════
print("=" * 50)
print("Génération des figures individuelles")
print("=" * 50)
...
print("\n" + "=" * 50)
print(f"8 figures sauvegardées dans :")
print(f"  {SCRIPT_DIR}")
print("=" * 50)
